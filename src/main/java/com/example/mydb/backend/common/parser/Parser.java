package com.example.mydb.backend.common.parser;
...
/*
 SQL 语句的 解析器 (Parser)，
 其主要功能是将输入的 SQL 字节流解析成不同的 SQL 操作对象，
 例如 SELECT、INSERT 等命令。
 */
public class Parser {
    /*
    Parse() 方法首先通过 Tokenizer 从输入字节流中获取第一个 token，
    根据不同的 token 确定 SQL 语句的类型，然后调用对应的解析方法。
     */
